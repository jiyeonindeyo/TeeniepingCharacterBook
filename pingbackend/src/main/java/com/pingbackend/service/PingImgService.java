...
public class PingImgService {
    ...
    서비스 책임?
    트랜젝션으로 묶여있기때문에 순서가 중요(파일 시스테멩 저장을 하고 DB에 저장해야됨)
    1. MultipartFile에 들어있는 이미지 파일 ==>파일시스템에 저장 ==> FileService 호출
    2. DB에 저장할 ItemImg 정보==> DB에 저장 ==? ItemRepository
     */
    public void savePingImg(PingImg pingImg, MultipartFile pingImgFile) throws Exception {
    ...
    public void updatePingImg(Long pingImg, MultipartFile pingImgFile) throws Exception {
        if(!pingImgFile.isEmpty()){
            PingImg savedPingImg = pingImgRepository.findById(pingImg)
                    .orElseThrow(EntityNotFoundException::new);

            //1. 기존에 파일 시스템에 저장되어있던 이미지 삭제
            if(!StringUtils.isEmpty(savedPingImg.getImgName())){
                fileService.deleteFile(pingImgFile + "/" + savedPingImg.getImgName());
            }
            ...
            String imgUrl = "/images/ping/" + imgName;
